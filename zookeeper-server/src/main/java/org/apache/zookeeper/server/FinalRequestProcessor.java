    ...
    public void processRequest(Request request) {
        ...
        synchronized (zks.outstandingChanges) {
            ...
            if (request.getHdr() != null) {
                ...
                while (!zks.outstandingChanges.isEmpty()
                    ...
                    //只会出现>= (当前请求是事务请求是，按照顺序应该是=，如果不是的话，应该是>)
                    if (cr.zxid < zxid) {
        ...
        if (request.type == OpCode.closeSession && connClosedByClient(request)) {
            ...
            if (closeSession(zks.serverCnxnFactory, request.sessionId) ||
                    closeSession(zks.secureServerCnxnFactory, request.sessionId)) {  //关闭ServerCnxn
                return;
            }
        }
        ...
        try {
            ...
            switch (request.type) {
                ...
                case OpCode.getChildren: {
                    ...
                    //将请求的数据反序列化到 GetChildrenRequest
                    ...
                    //如果 getChildrenRequest 中的 watch 布尔变量是true的话，传入cnxn对象", 因为cnxn对象实现了watcher接口方便后面的时候回调
